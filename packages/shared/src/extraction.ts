...
export function extractBySelectors(html: string, selectors: Record<string, string>): Record<string, unknown> {
...
export function extractByJsonSchema(html: string, schema: Record<string, unknown>): Record<string, unknown> {
...
export async function extractByPlugin(pluginPath: string, ctx: ExtractorContext): Promise<unknown> {
  const abs = path.isAbsolute(pluginPath) ? pluginPath : path.join(process.cwd(), pluginPath);
  const mod = (await import(pathToFileURL(abs).toString())) as Partial<ExtractorPlugin> & {
    default?: ExtractorPlugin | ((context: ExtractorContext) => Promise<unknown> | unknown);
  };
  ...
  if (mod.default && typeof mod.default === 'object' && typeof mod.default.extract === 'function') {
