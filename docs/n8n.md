...
```
*(Note: Setting `"wait": true` tells the Kryfto API to hold the connection open until the scrape finishes and return the final data immediately. This is simpler than webhooks for fast jobs under 30 seconds).*
...
You can use this inside an n8n HTTP Request node to find domains *before* you crawl them.
...
```
*(Note: LLM extraction requires you to have `OPENAI_API_KEY` configured in Kryfto's `.env` file).*
