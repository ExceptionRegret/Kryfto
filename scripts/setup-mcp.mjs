...
const MCP_INDEX_PATH = path.join(REPO_ROOT, 'packages', 'mcp-server', 'dist', 'index.js');
...
async function main() {
    ...
    let token = await rl.question('Enter Kryfto API Token (leave empty to use .env KRYFTO_API_TOKEN): ');
    token = token.trim();
    if (!token) {
        try {
...
`;
        console.log(toml);
        console.log('\n👉 Instructions:');
        console.log('Copy the above TOML and append it to your project\'s `.codex/config.toml` file, or your global `~/.codex/config.toml`.');
    }
    else {
        ...
        if (clientChoice === '1') {
            ...
            console.log('\n👉 Instructions:');
            console.log('For Claude Desktop: Copy this into `~/Library/Application Support/Claude/claude_desktop_config.json` (Mac) or `%APPDATA%\\Claude\\claude_desktop_config.json` (Windows).');
            console.log('For Cursor: Go to Cursor Settings > Features > MCP, click "+ Add new MCP server", select type "command", command `node`, and pass the absolute path/env variables shown above.');
        } else if (clientChoice === '3') {
            // RooCode often just wants the inner object
            console.log(JSON.stringify(jsonConfig, null, 2));
            console.log('\n👉 Instructions:');
            console.log('Copy this into your global `mcp_settings.json` file used by RooCode/Cline.');
        } else {
            console.log(JSON.stringify(jsonConfig, null, 2));
        }
    }
