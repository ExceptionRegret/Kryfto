...
import { db, generateApiToken, hashToken, pool, runMigrations } from './db/client.js';
import {
  apiTokens,
  artifactBlobs,
...
const app = Fastify({
  logger: {
    level: LOG_LEVEL,
    redact: ['req.headers.authorization', '*.token', '*.secret', '*.password', '*.apiKey'],
  },
  genReqId: (req) => {
    const fromHeader = req.headers['x-request-id'];
    return typeof fromHeader === 'string' && fromHeader.trim() ? fromHeader : randomUUID();
  },
});
...
async function ensureBootstrapData(): Promise<void> {
  ...
  const bootstrapToken = process.env.KRYFTO_BOOTSTRAP_ADMIN_TOKEN ?? process.env.KRYFTO_API_TOKEN;
  if (!bootstrapToken) {
    return;
  }
  ...
  const existing = await db
    ...
    .from(apiTokens)
    .where(and(eq(apiTokens.tokenHash, hashToken(bootstrapToken)), isNull(apiTokens.revokedAt)))
    .limit(1);
...
async function resolveAuth(request: { headers: Record<string, unknown> }): Promise<AuthContext | null> {
  const header = request.headers.authorization;
  const token = parseBearerToken(typeof header === 'string' ? header : undefined);
...
const crawlQueue = new Queue('collector-crawls', { connection: redisConnection });
...
await app.register(rateLimit, {
  ...
  keyGenerator: (req) => {
    ...
    const tokenHash = token ? createHash('sha1').update(token).digest('hex') : 'anonymous';
...
app.addHook('onResponse', async (req, reply) => {
  ...
  if (req.requestStartNs) {
    const durationSec = Number(process.hrtime.bigint() - req.requestStartNs) / 1_000_000_000;
    latencyHistogram.observe({ method: req.method, route, status }, durationSec);
  }
  req.apiSpan?.end();
});

app.addHook('preHandler', async (req, reply) => {
  const publicPrefixes = ['/v1/healthz', '/v1/readyz', '/v1/metrics', '/docs', '/documentation', '/docs/openapi.yaml'];
  if (publicPrefixes.some((prefix) => req.url.startsWith(prefix))) {
    return;
  }

  const resolvedAuth = await resolveAuth(req as { headers: Record<string, unknown> });
  if (resolvedAuth) {
  ...
  const hasDownloadToken = typeof query.downloadToken === 'string' && query.downloadToken.length > 0;

  if (isArtifactDownload && hasDownloadToken) {
    return;
  }

  reply.status(401).send(createErrorResponse('AUTH_UNAUTHORIZED', 'Unauthorized', req.id));
});
...
app.setErrorHandler((error, req, reply) => {
  ...
  if (error instanceof Error && error.message === 'AUTH_FORBIDDEN') {
    reply.status(403).send(createErrorResponse('AUTH_FORBIDDEN', 'Forbidden', req.id));
    return;
  }

  req.log.error({ err: error }, 'Unhandled error');
  reply.status(500).send(createErrorResponse('INTERNAL_ERROR', 'Internal server error', req.id));
});
...
app.post('/v1/admin/tokens', async (req, reply) => {
  ...
  if (!parsed.success) {
    return reply.status(400).send(createErrorResponse('VALIDATION_ERROR', 'Invalid token request', req.id, parsed.error.flatten()));
  }
  ...
  return reply.status(201).send({ token: tokenPlain, tokenId: inserted[0]?.id });
});
...
app.post('/v1/jobs', async (req, reply) => {
  ...
  const idempotencyKey = typeof idemKeyRaw === 'string' ? idemKeyRaw.trim() : undefined;

  const parsed = JobCreateRequestSchema.safeParse(req.body);
  if (!parsed.success) {
    return reply.status(400).send(createErrorResponse('VALIDATION_ERROR', 'Invalid job request', req.id, parsed.error.flatten()));
  }
  ...
  return tracer.startActiveSpan('api.create_job', async (span) => {
    try {
      ...
      if (parsed.data.recipeId) {
        ...
        if (!recipe) {
          return reply.status(404).send(createErrorResponse('RECIPE_NOT_FOUND', 'Recipe not found', req.id));
        }
      } else {
        recipe = registry.find((item) => recipeMatchesUrl(item, parsed.data.url));
      }
      ...
      const mergedRequest = JobCreateRequestSchema.parse({
        ...
        options: {
          ...parsed.data.options,
          ...(parsed.data.options?.requiresBrowser === undefined && recipe ? { requiresBrowser: recipe.requiresBrowser } : {}),
        },
      ...
      if (idempotencyKey) {
        const idemRows = await db
          ...
          .where(and(eq(idempotencyKeys.projectId, auth.projectId), eq(idempotencyKeys.key, idempotencyKey)))
          .limit(1);
        ...
        if (idem) {
          if (idem.requestHash !== requestHash) {
            return reply
              .status(409)
              .send(createErrorResponse('IDEMPOTENCY_CONFLICT', 'Idempotency key reused with different payload', req.id));
          }

          const existingJob = await db
            .select()
            .from(jobs)
            .where(and(eq(jobs.id, idem.jobId), eq(jobs.projectId, auth.projectId)))
            .limit(1);
      ...
      await writeAuditLog({
        ...
        requestId: req.id,
        ip: req.ip,
        details: { url: mergedRequest.url, recipeId: mergedRequest.recipeId ?? null },
      });
...
app.get('/v1/jobs/:jobId', async (req, reply) => {
  ...
  const row = rows[0];
  if (!row) {
    return reply.status(404).send(createErrorResponse('NOT_FOUND', 'Job not found', req.id));
  }
...
app.get('/v1/jobs/:jobId/logs', async (req, reply) => {
  ...
  const emitLogs = async () => {
    const rows = await db
      .select({ id: jobLogs.id, level: jobLogs.level, message: jobLogs.message, meta: jobLogs.meta, createdAt: jobLogs.createdAt })
      .from(jobLogs)
  ...
  const timer = setInterval(async () => {
    ...
    try {
      ...
      reply.raw.write(`data: ${JSON.stringify({ message: String(error) })}\n\n`);
    }
  }, 1000);
...
app.get('/v1/jobs/:jobId/artifacts', async (req, reply) => {
  ...
  const rows = await db
    ...
    .where(and(eq(artifacts.jobId, jobId), eq(artifacts.projectId, auth.projectId)))
...
app.get('/v1/artifacts/:artifactId', async (req, reply) => {
  ...
  if (!artifact) {
    return reply.status(404).send(createErrorResponse('NOT_FOUND', 'Artifact not found', req.id));
  }

  let auth = req.auth;
  if (auth) {
    if (auth.projectId !== artifact.projectId) {
      return reply.status(403).send(createErrorResponse('AUTH_FORBIDDEN', 'Forbidden', req.id));
    }
  } else {
    if (!query.downloadToken) {
      return reply.status(401).send(createErrorResponse('AUTH_UNAUTHORIZED', 'Unauthorized', req.id));
    }

    const tokenRows = await db
      .select()
      .from(artifactDownloadTokens)
      .where(and(eq(artifactDownloadTokens.token, query.downloadToken), eq(artifactDownloadTokens.artifactId, artifactId)))
      .limit(1);

    const token = tokenRows[0];
    if (!token || token.expiresAt.getTime() < Date.now()) {
      return reply.status(401).send(createErrorResponse('AUTH_UNAUTHORIZED', 'Download token expired or invalid', req.id));
    }
...
app.post('/v1/extract', async (req, reply) => {
  ...
  if (!parsed.success) {
    return reply.status(400).send(createErrorResponse('VALIDATION_ERROR', 'Invalid extract request', req.id, parsed.error.flatten()));
  }
  ...
  if (parsed.data.mode === 'selectors') {
    ...
    const pluginPath = path.isAbsolute(plugin) ? plugin : resolveRepoPath('recipes', 'plugins', plugin);
...
app.post('/v1/search', async (req, reply) => {
  ...
  if (!parsed.success) {
    return reply.status(400).send(createErrorResponse('VALIDATION_ERROR', 'Invalid search request', req.id, parsed.error.flatten()));
  }
  const querySha = createHash('sha256').update(parsed.data.query).digest('hex');
  const locale = localeParts(parsed.data.locale);
  const searchTimeoutMs = Number(process.env.KRYFTO_SEARCH_TIMEOUT_MS ?? 20_000);
  ...
  let results: Array<{ title: string; url: string; snippet?: string; rank: number }> = [];
  try {
    switch (parsed.data.engine) {
      case 'duckduckgo': {
        ...
        results = parseDuckDuckGoSearchResults(await fetchHtml(searchUrl), parsed.data.limit);
        break;
      }
      case 'yahoo': {
        ...
        results = parseYahooSearchResults(await fetchHtml(searchUrl), parsed.data.limit);
        break;
      }
      case 'bing': {
        ...
        if (bingKey) {
          try {
            ...
            url.searchParams.set('safeSearch', safeSearchToBing(parsed.data.safeSearch));
            url.searchParams.set('mkt', locale.mkt);
            const payload = await fetchJson(url.toString(), { 'Ocp-Apim-Subscription-Key': bingKey });
            ...
            results = parseBingHtmlSearchResults(await fetchHtml(searchUrl), parsed.data.limit);
          }
        } else {
          const searchUrl = buildBingHtmlSearchUrl({
            query: parsed.data.query,
      ...
      case 'google': {
        ...
        if (googleKey && googleCx) {
          try {
            ...
            url.searchParams.set('safe', safeSearchToGoogle(parsed.data.safeSearch));
        ...
        results = parseGoogleHtmlSearchResults(await fetchHtml(searchUrl), parsed.data.limit);
        break;
      }
      case 'brave': {
        ...
        if (braveKey) {
          try {
            ...
            url.searchParams.set('q', parsed.data.query);
            url.searchParams.set('count', String(parsed.data.limit));
            url.searchParams.set('safesearch', safeSearchToBrave(parsed.data.safeSearch));
            url.searchParams.set('country', locale.region.toUpperCase());
            url.searchParams.set('search_lang', locale.language);
            const payload = await fetchJson(url.toString(), { 'X-Subscription-Token': braveKey });
        ...
        results = parseBraveHtmlSearchResults(await fetchHtml(searchUrl), parsed.data.limit);
        break;
      }
      default: {
        return reply.status(400).send(createErrorResponse('SEARCH_ENGINE_UNSUPPORTED', 'Unsupported search engine', req.id));
      }
    }
  } catch (error) {
    return reply
      .status(502)
      .send(createErrorResponse('SEARCH_UPSTREAM_ERROR', `Search provider request failed: ${String(error)}`, req.id));
  }
...
app.post('/v1/crawl', async (req, reply) => {
  ...
  if (!parsed.success) {
    return reply.status(400).send(createErrorResponse('VALIDATION_ERROR', 'Invalid crawl request', req.id, parsed.error.flatten()));
  }
  ...
  return reply.status(202).send({ crawlId, state: 'queued', requestId: req.id });
});
...
app.get('/v1/crawl/:crawlId', async (req, reply) => {
  ...
  const rows = await db
    .select()
    .from(crawlRuns)
    .where(and(eq(crawlRuns.id, crawlId), eq(crawlRuns.projectId, auth.projectId)))
    .limit(1);

  const crawl = rows[0];
  if (!crawl) {
    return reply.status(404).send(createErrorResponse('NOT_FOUND', 'Crawl not found', req.id));
  }
...
app.post('/v1/recipes/validate', async (req) => {
  ...
  const payload = body && typeof body === 'object' && 'recipe' in body ? body.recipe : req.body;
...
app.post('/v1/recipes', async (req, reply) => {
  ...
  if (!parsed.success) {
    return reply.status(400).send(createErrorResponse('VALIDATION_ERROR', 'Invalid recipe payload', req.id, parsed.error.flatten()));
  }
