...
export const crawlStateEnum = pgEnum("crawl_state", [
  "queued",
  "running",
  "succeeded",
  "failed",
...
export const idempotencyKeys = pgTable(
  ...
  (table) => ({
    pk: primaryKey({
      columns: [table.projectId, table.key],
...
export const jobs = pgTable(
  ...
  (table) => ({
    projectStateIdx: index("jobs_project_state_idx").on(
      table.projectId,
      table.state
    ),
...
export const artifactDownloadTokens = pgTable(
  ...
  (table) => ({
    artifactIdx: index("artifact_download_tokens_artifact_idx").on(
      table.artifactId
    ),
    projectIdx: index("artifact_download_tokens_project_idx").on(
      table.projectId
    ),
  })
);
...
export const crawlRuns = pgTable(
  "crawl_runs",
  {
    ...
    stats: jsonb("stats")
      .notNull()
      .default({ queued: 0, running: 0, succeeded: 0, failed: 0 }),
    requestId: varchar("request_id", { length: 128 }).notNull(),
    errorMessage: text("error_message"),
    createdAt: timestamp("created_at", { withTimezone: true })
      .defaultNow()
...
export const crawlNodes = pgTable(
  ...
  (table) => ({
    crawlDepthIdx: index("crawl_nodes_crawl_depth_idx").on(
      table.crawlId,
      table.depth
    ),
  })
