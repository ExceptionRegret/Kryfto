...
export function roleAllowed(role: AuthContext['role'], allowedRoles: Array<AuthContext['role']>): boolean {
  return allowedRoles.includes(role);
}

export function requireRole(auth: AuthContext | undefined, allowedRoles: Array<AuthContext['role']>): AuthContext {
  if (!auth) {
